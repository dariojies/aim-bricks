...
export function useTweaks(defaults: Record<string, any>): [Record<string, any>, (keyOrEdits: string | Record<string, any>, val?: any) => void] {
  const [values, setValues] = React.useState(defaults);
  const setTweak = React.useCallback((keyOrEdits: string | Record<string, any>, val?: any) => {
    const edits = typeof keyOrEdits === 'object' && keyOrEdits !== null
      ? keyOrEdits : { [keyOrEdits as string]: val };
    setValues((prev: Record<string, any>) => ({ ...prev, ...edits }));
...
type RadioOption = string | { value: any; label: string };

export function TweakRadio({ label, value, options, onChange }: {
  label: string; value: any; options: RadioOption[]; onChange: (v: any) => void;
}) {
...
export function TweakSelect({ label, value, options, onChange }: {
  label: string; value: any; options: RadioOption[]; onChange: (v: string) => void;
}) {
  return (
    <TweakRow label={label}>
...
export function TweakColor({ label, value, options, onChange }: {
  label: string; value: any; options?: (string | string[])[]; onChange: (v: any) => void;
}) {
    ...
    <TweakRow label={label}>
      <div className="twk-chips" role="radiogroup">
        {options.map((o, i) => {
          ...
          const on = key(o) === cur;
          const isLight = __twkIsLight(hero);
          return (
