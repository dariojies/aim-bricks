...
export const Ranking: React.FC = () => {
  ...
  const getSortedRanking = () => {
    return [...ranking]
      .filter(entry => entry[timeframe] > 0)
  ...
  const renderTopThree = () => {
    ...
    return (
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'flex-end', gap: '1rem', marginTop: '3rem', marginBottom: '4rem', flexWrap: 'wrap' }}>
        {podium.map((entry, i) => {
          if (!entry) return <div key={i} style={{ width: '120px' }}></div>;
            ...
            <div key={entry.id} className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '120px', animationDelay: `${position * 0.1}s` }}>
              <div style={{ position: 'relative', marginBottom: '1rem' }}>
                <div style={{ 
                  ...
                  fontSize: '2rem', fontWeight: 'bold', color: 'var(--text-muted)'
                }}>
                  {entry.avatar ? (
                ...
                <div style={{ 
                  ...
                  background: color, color: '#000', width: '30px', height: '30px', borderRadius: '50%',
                ...
                {position === 1 && (
                  <Crown size={32} color="#FBBF24" style={{ position: 'absolute', top: '-25px', left: '50%', transform: 'translateX(-50%)' }} />
                )}
              </div>
              ...
              <div style={{ 
                width: '100%', height, background: `linear-gradient(to top, var(--surface-border), transparent)`,
                borderRadius: '8px 8px 0 0', border: `1px solid var(--surface-border)`, borderBottom: 'none'
              }}></div>
            </div>
          );
        })}
    ...
    <div style={{ maxWidth: '800px', margin: '0 auto', width: '100%', padding: '0 1rem' }}>
      <div style={{ textAlign: 'center', marginBottom: '3rem' }}>
        ...
        <p style={{ color: 'var(--text-muted)', fontSize: '1.1rem', marginTop: '1rem' }}>
          Los alumnos más activos montando sets de Aim Brickslab.
        </p>
      </div>

      <div style={{ display: 'flex', justifyContent: 'center', gap: '1rem', marginBottom: '2rem' }}>
        <button 
          className={`btn ${timeframe === 'monthly' ? 'btn-primary' : 'btn-outline'}`}
          onClick={() => setTimeframe('monthly')}
        >
          Mes Actual
        </button>
        <button 
          className={`btn ${timeframe === 'yearly' ? 'btn-primary' : 'btn-outline'}`}
          onClick={() => setTimeframe('yearly')}
        >
          Este Año
        </button>
        <button 
          className={`btn ${timeframe === 'allTime' ? 'btn-primary' : 'btn-outline'}`}
      ...
      {loading ? (
        ...
        <div className="glass-panel" style={{ textAlign: 'center', padding: '4rem', color: 'var(--text-muted)' }}>
          Aún no hay suficientes datos para este período. ¡Anímate a construir un set!
        </div>
      ) : (
        <>
          {renderTopThree()}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              {sortedRanking.slice(3).map((entry, index) => (
                <div key={entry.id} style={{ 
                  ...
                  background: 'var(--background)', borderRadius: '8px', border: '1px solid var(--surface-border)' 
                }}>
                  <div style={{ width: '40px', fontWeight: 'bold', color: 'var(--text-muted)', fontSize: '1.2rem' }}>
                    #{index + 4}
                  </div>
                  
                  <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: '1rem' }}>
                    <div style={{ 
                      width: '40px', height: '40px', borderRadius: '50%', background: 'var(--surface-border)', 
              ...
              {sortedRanking.length <= 3 && (
                <p style={{ textAlign: 'center', color: 'var(--text-muted)', padding: '1rem' }}>No hay más constructores en este período.</p>
              )}
            </div>
          </div>
        </>
